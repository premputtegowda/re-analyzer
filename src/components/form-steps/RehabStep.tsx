...
export default function RehabStep() {
  ...
  const addLostRevenueItem = () => {
    const currentData = rehabData?.lostRevenueAndCosts || [];
    if (currentData.length === 0) {
      setValue('rehab.lostRevenueAndCosts', [
        { year: 1, items: [{ category: '', amount: 0 }] }
      ]);
    } else {
      const updatedData = [...currentData];
      updatedData[0] = {
        ...updatedData[0],
        items: [...updatedData[0].items, { category: '', amount: 0 }]
      };
      setValue('rehab.lostRevenueAndCosts', updatedData);
    }
  };

  const removeLostRevenueItem = (itemIndex: number) => {
    const currentData = rehabData?.lostRevenueAndCosts || [];
    if (currentData.length > 0) {
      const updatedData = [...currentData];
      updatedData[0] = {
        ...updatedData[0],
        items: updatedData[0].items.filter((_, index) => index !== itemIndex)
      };
      setValue('rehab.lostRevenueAndCosts', updatedData);
    }
  };

  const lostRevenueItems = rehabData?.lostRevenueAndCosts?.[0]?.items || [];
    ...
    <>
      ...
      {lostRevenueItems.length === 0 ? (
        ...
        <>
          ...
          {lostRevenueItems.map((item, index) => (
            <div key={index}>
              {index > 0 && <hr className="my-4 border-slate-200" />}
              <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-4">
                <div className="w-full">
                  <input
                    {...register(`rehab.lostRevenueAndCosts.0.items.${index}.category`, {
                      required: 'Lost revenue/cost category is required'
                    })}
                    placeholder="e.g., Lost Revenue, Utilities"
                    className={`w-full px-3 py-2 border rounded-md shadow-sm ${
                      errors.rehab?.lostRevenueAndCosts?.[0]?.items?.[index]?.category ? 'border-red-500' : 'border-slate-300'
                    }`}
                  />
                  {errors.rehab?.lostRevenueAndCosts?.[0]?.items?.[index]?.category && (
                    <p className="mt-1 text-sm text-red-600">{errors.rehab?.lostRevenueAndCosts?.[0]?.items?.[index]?.category?.message}</p>
                  )}
                </div>
                <div className="w-full">
                  <input
                    type="number"
                    {...register(`rehab.lostRevenueAndCosts.0.items.${index}.amount`, {
                      valueAsNumber: true,
                    ...
                    placeholder="Amount"
                    className={`w-full px-3 py-2 border rounded-md shadow-sm ${
                      errors.rehab?.lostRevenueAndCosts?.[0]?.items?.[index]?.amount ? 'border-red-500' : 'border-slate-300'
                    }`}
                  />
                  {errors.rehab?.lostRevenueAndCosts?.[0]?.items?.[index]?.amount && (
                    <p className="mt-1 text-sm text-red-600">{errors.rehab?.lostRevenueAndCosts?.[0]?.items?.[index]?.amount?.message}</p>
                  )}
                </div>
                <button
                  type="button"
                  onClick={() => removeLostRevenueItem(index)}
                  className="p-2 rounded-md hover:bg-red-100 self-start sm:self-center"
